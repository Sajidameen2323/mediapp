...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <form action="{{ route('admin.doctors.update', $doctor) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl border dark:border-gray-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">Update professional credentials and qualifications</p>
            </div>
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                ...
                <div class="col-span-full">
                    ...
                    <input type="file" 
                           ...
                           accept="image/*" 
                           class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                    <p class="text-sm text-gray-500 mt-1">Upload new image to replace current one (JPEG, PNG, GIF, max 2MB)</p>
                    @error('profile_image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-gray-700 dark:to-gray-600">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Update the services this doctor can provide</p>
                    </div>
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" id="services-grid">
                    @foreach($services as $service)
                        @php
                            $isSelected = in_array($service->id, old('services', $doctor->services->pluck('id')->toArray()));
                        @endphp
                        <div class="service-card group relative overflow-hidden rounded-xl border border-gray-200 dark:border-gray-600 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-700 hover:shadow-lg hover:scale-[1.02] transition-all duration-300 cursor-pointer {{ $isSelected ? 'ring-2 ring-purple-500 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900 dark:to-pink-900' : '' }}">
                            <div class="p-4">
                                <div class="flex items-start justify-between mb-3">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-purple-100 to-pink-100 dark:from-purple-900 dark:to-pink-900 flex items-center justify-center">
                                            <i class="fas fa-stethoscope text-purple-600 dark:text-purple-400 text-sm"></i>
                                        </div>
                                        <div class="flex-1">
                                            <h4 class="text-sm font-semibold text-gray-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors duration-200">
                                                {{ $service->name }}
                                            </h4>
                                            @if($service->category)
                                                <span class="inline-block px-2 py-1 mt-1 text-xs font-medium bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-300 rounded-full">
                                                    {{ $service->category }}
                                                </span>
                                            @endif
                                        </div>
                                    ...
                                    <div class="service-checkbox-container">
                                        ...
                                        <label for="service_{{ $service->id }}" class="sr-only">Select {{ $service->name }}</label>
                                    </div>
                                </div>
                                
                                <div class="space-y-2">
                                    @if($service->description)
                                        <p class="text-xs text-gray-600 dark:text-gray-400 line-clamp-2">{{ $service->description }}</p>
                                    @endif
                                    
                                    <div class="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-600">
                                        @if($service->price)
                                            <div class="flex items-center space-x-1">
                                                <i class="fas fa-dollar-sign text-xs text-green-600 dark:text-green-400"></i>
                                                <span class="text-sm font-bold text-green-600 dark:text-green-400">${{ number_format($service->price, 2) }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($service->duration)
                                            <div class="flex items-center space-x-1">
                                                ...
                                                <span class="text-xs text-gray-600 dark:text-gray-400">{{ $service->duration }} min</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <!-- Selection Indicator -->
                            <div class="absolute top-2 right-2 w-6 h-6 rounded-full {{ $isSelected ? 'bg-green-500 border-green-500' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600' }} border-2 flex items-center justify-center transition-all duration-200 service-indicator">
                                <i class="fas fa-check text-xs text-green-600 dark:text-green-400 {{ $isSelected ? 'opacity-100' : 'opacity-0' }} service-check-icon transition-opacity duration-200"></i>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <!-- Services Summary -->
                <div class="mt-6 p-4 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-gray-700 dark:to-gray-600 rounded-xl border border-purple-200 dark:border-gray-600">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">Selected services: <span id="selected-services-count" class="font-medium text-purple-600 dark:text-purple-400">{{ $doctor->services->count() }}</span> of {{ $services->count() }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-xs text-gray-600 dark:text-gray-400">Total potential revenue:</p>
                            <p class="text-lg font-bold text-purple-600 dark:text-purple-400" id="total-revenue">${{ number_format($doctor->services->sum('price'), 2) }}</p>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl border dark:border-gray-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-600">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Update doctor's availability for appointments</p>
                    </div>
                    <div class="flex items-center space-x-3">
            ...
            <div class="p-6" id="schedule-container">
                <div class="space-y-3">
                    @foreach(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $day)
                        ...
                        <div class="schedule-day-container group hover:shadow-lg transition-all duration-300 border border-gray-200 dark:border-gray-600 rounded-xl overflow-hidden">
                                ...
                                <div class="flex items-center space-x-4 flex-1">
                                    ...
                                    <div class="flex items-center ml-auto">
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            ...
                                            <input type="checkbox" name="schedules[{{ $day }}][is_available]" id="schedule_{{ $day }}_available" value="1" class="sr-only schedule-checkbox" data-day="{{ $day }}" {{ $schedule && $schedule->is_available ? 'checked' : '' }}>
                                            <div class="relative w-14 h-7 bg-gray-200 dark:bg-gray-600 rounded-full transition-colors duration-300 ease-in-out toggle-switch {{ $schedule && $schedule->is_available ? 'bg-green-500' : '' }}">
                                                <div class="absolute top-0.5 left-[4px] bg-white border border-gray-300 rounded-full h-6 w-6 transition-transform duration-300 ease-in-out shadow-lg toggle-thumb" style="transform: translateX({{ $schedule && $schedule->is_available ? '28px' : '4px' }})">
                                                    <i class="fas fa-check text-xs text-green-600 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 {{ $schedule && $schedule->is_available ? 'opacity-100' : 'opacity-0' }} transition-opacity duration-300 check-icon"></i>
                                                </div>
                                            </div>
                                            <span class="ml-3 text-sm font-medium text-gray-700 dark:text-gray-300">
                                                <span class="availability-text {{ $schedule && $schedule->is_available ? 'text-green-600 dark:text-green-400' : 'text-gray-600 dark:text-gray-400' }}">{{ $schedule && $schedule->is_available ? 'Available' : 'Unavailable' }}</span>
                                            </span>
                                        </label>
                                    </div>
                                </div>
                            ...
                            <div class="schedule-times bg-white dark:bg-gray-800 p-4 border-t border-gray-100 dark:border-gray-700" style="display: {{ $schedule && $schedule->is_available ? 'block' : 'none' }};">
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div class="relative">
                                            <input type="time" name="schedules[{{ $day }}][start_time]" 
                                                   ...
                                                   class="block w-full px-4 py-3 pl-10 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm focus:ring-2 focus:ring-{{ $color }}-500 focus:border-{{ $color }}-500 dark:bg-gray-700 dark:text-white transition-all duration-200 text-sm font-medium">
                                ...
                                <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <button type="button" class="time-preset text-xs px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200" data-start="09:00" data-end="17:00">
                                            9 AM - 5 PM
                                        </button>
                                        <button type="button" class="time-preset text-xs px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200" data-start="08:00" data-end="16:00">
                                            8 AM - 4 PM
                                        </button>
                                        <button type="button" class="time-preset text-xs px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200" data-start="10:00" data-end="18:00">
                                            10 AM - 6 PM
                                        </button>
                                        <button type="button" class="time-preset text-xs px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200" data-start="14:00" data-end="22:00">
                                            2 PM - 10 PM
                                        </button>
                                    </div>
                                </div>
                ...
                <!-- Schedule Summary -->
                <div class="mt-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-600 rounded-xl border border-blue-200 dark:border-gray-600">
                    <div class="flex items-center justify-between">
                        <div>
                            <h4 class="text-sm font-semibold text-gray-900 dark:text-white flex items-center">
                            ...
                            <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">Total available days: <span id="available-days-count" class="font-medium text-blue-600 dark:text-blue-400">0</span></p>
                        </div>
                        <div class="text-right">
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" 
                    class="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-indigo-700 transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    scheduleCheckboxes.forEach(checkbox => {
        ...
        checkbox.addEventListener('change', function() {
            ...
            if (this.checked) {
                ...
                const startTime = timesContainer.querySelector('input[type="time"]:first-of-type');
                const endTime = timesContainer.querySelector('input[type="time"]:last-of-type');
    ...
    serviceCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            if (this.checked) {
                card.classList.add('ring-2', 'ring-purple-500', 'bg-gradient-to-br', 'from-purple-50', 'to-pink-50', 'dark:from-purple-900', 'dark:to-pink-900');
                indicator.classList.add('bg-green-500', 'border-green-500');
                indicator.classList.remove('bg-white', 'dark:bg-gray-800', 'border-gray-300', 'dark:border-gray-600');
                checkIcon.classList.remove('opacity-0');
                checkIcon.classList.add('opacity-100');
            } else {
                card.classList.remove('ring-2', 'ring-purple-500', 'bg-gradient-to-br', 'from-purple-50', 'to-pink-50', 'dark:from-purple-900', 'dark:to-pink-900');
                indicator.classList.remove('bg-green-500', 'border-green-500');
                indicator.classList.add('bg-white', 'dark:bg-gray-800', 'border-gray-300', 'dark:border-gray-600');
                checkIcon.classList.add('opacity-0');
                checkIcon.classList.remove('opacity-100');
            }
