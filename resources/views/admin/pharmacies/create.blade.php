...
@section('title', 'Add New Pharmacy - Medi App')
...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Add New Pharmacy</h1>
            <p class="mt-2 text-gray-600 dark:text-gray-400">Register a new pharmacy in the platform</p>
        </div>
        ...
        <form action="{{ route('admin.pharmacies.store') }}" method="POST" class="space-y-8">
            @csrf
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            <label for="pharmacy_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        ...
                        <div>
                            <label for="pharmacist_in_charge" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                <i class="fas fa-user-md mr-2 text-purple-500"></i>Pharmacist in Charge
                            </label>
                            <input type="text" name="pharmacist_in_charge" id="pharmacist_in_charge" value="{{ old('pharmacist_in_charge') }}"
                                class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white transition-all duration-200">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white transition-all duration-200">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="p-6 space-y-6">
                    <div>
                        ...
                        <textarea name="address" id="address" rows="3" required
                            class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white transition-all duration-200">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="country" id="country" value="{{ old('country', 'Sri Lanka') }}" required
                                class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white transition-all duration-200">
                            @error('country')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="time" name="opening_time" id="opening_time" value="{{ old('opening_time', '08:00') }}" required
                                class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white transition-all duration-200">
                            @error('opening_time')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="time" name="closing_time" id="closing_time" value="{{ old('closing_time', '22:00') }}" required
                                class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white transition-all duration-200">
                            @error('closing_time')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                            @php
                                ...
                                $oldWorkingDays = old('working_days', ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']);
                            @endphp
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl border dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-6">
                    <div>
                        <label for="specializations" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                            <i class="fas fa-star mr-2 text-purple-500"></i>Specializations
                        </label>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            @php
                                $specializations = [
                                    'General Pharmacy', 'Clinical Pharmacy', 'Compounding Pharmacy', 'Veterinary Pharmacy',
                                    'Oncology Pharmacy', 'Pediatric Pharmacy', 'Geriatric Pharmacy', 'Psychiatric Pharmacy',
                                    'Hospital Pharmacy', 'Community Pharmacy', 'Industrial Pharmacy', 'Nuclear Pharmacy'
                                ];
                            ...
                            @foreach($specializations as $specialization)
                                <label class="flex items-center p-3 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-all duration-200">
                                    <input type="checkbox" name="specializations[]" value="{{ $specialization }}"
                                           ...
                                           class="h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded">
                                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">{{ $specialization }}</span>
                                </label>
                            @endforeach
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <input type="checkbox" name="home_delivery_available" id="home_delivery_available" value="1" 
                                   {{ old('home_delivery_available') ? 'checked' : '' }}
                                   class="h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded">
                            <label for="home_delivery_available" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">
                    ...
                    <div id="delivery_info" class="{{ old('home_delivery_available') ? '' : 'hidden' }}">
                        <label for="home_delivery_fee" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            <i class="fas fa-dollar-sign mr-2 text-purple-500"></i>Delivery Fee (LKR)
                        </label>
                        <input type="number" name="home_delivery_fee" id="home_delivery_fee" value="{{ old('home_delivery_fee', '0') }}" 
                               min="0" step="0.01"
                            class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white transition-all duration-200">
